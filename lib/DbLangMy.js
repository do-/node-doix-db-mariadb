...
module.exports = class DbLangMy extends DbLang {
	...
	* genReCreateTriggers () {
		...
		for (const o of this.model.objects ()) if (o instanceof DbTable && 'triggers' in o)

			for (const {qName, phase, action, sql} of o.triggers)

				yield `CREATE OR REPLACE TRIGGER ${qName} ${phase} ON ${o.qName} ${action} ${sql}`
